...
fix_repository_issues() {
  ...
  if ! apt update -qq; then
    log_warning "Problemas detectados, tentando correção..."
    apt-get clean
...
prompt_config() {
  ...
  echo -e "${BLUE}🌐 Domínio (opcional):${NC}"
  read -p "Digite o domínio (ou deixe em branco): " input_domain
  ...
  read -p "Diretório de instalação (padrão: /opt/ubuntu-server-admin): " input_dir
...
clone_repository() {
  ...
  if [[ -d "$INSTALL_DIR" ]]; then
    log_warning "Diretório existe. Backup..."
...
setup_frontend() {
  ...
  log_info "Instalando dependências do Node..."
...
EOF
  ...
  log_info "Compilando Angular (produção, não-interativo)..."
  if sudo -u "$SERVICE_USER" npx ng build --configuration production --no-interactive; then
    ...
    log_warning "Build básico concluído"
  fi
  ...
  if [[ -f "/var/www/html/serveradmin/browser/index.html" ]]; then
    ...
    log_error "Arquivos do frontend não encontrados no destino"
    exit 1
  fi
}
...
setup_ssl() {
  ...
  if [[ "$USE_DOMAIN" == true && -n "$DOMAIN" ]]; then
    if certbot --nginx -d "$DOMAIN" --non-interactive --agree-tos --email "$SSL_EMAIL"; then
      ...
      log_warning "Falha ao obter certificado SSL. Você pode tentar depois com: certbot --nginx -d $DOMAIN"
    fi
  else
    log_warning "SSL pulado: domínio não informado"
  fi
}
...
create_admin_script() {
  log "Criando utilitário serveradmin..."
...
case "$1" in
  ...
  update)
    ...
    if sudo -u serveradmin npx ng build --configuration production --no-interactive; then
      echo "✅ Build concluído"
    else
      echo "⚠️ Build otimizado falhou, tentando básico..."
      sudo -u serveradmin npx ng build --aot=false --optimization=false --no-interactive
    fi
    if [[ -d dist/ubuntu-server-admin/browser ]]; then
      ...
      echo "❌ Arquivos de build não encontrados"; exit 1
    fi
  ...
  health)
    for s in ubuntu-server-admin postgresql redis-server nginx; do
      systemctl is-active --quiet "$s" && echo "✅ $s: Ativo" || echo "❌ $s: Inativo"
    done
    ;;
  test)
...
EOF
  ...
  log "Utilitário criado em /usr/local/bin/serveradmin"
}
...
parse_arguments() {
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      *) echo -e "${RED}Parâmetro desconhecido: $1${NC}"; show_help; exit 1;;
    esac
  done
}
...
show_help() {
  ...
  echo "Uso: sudo bash install.sh [--domain DOMÍNIO] [--email EMAIL] [--directory DIR] [--skip-ssl] [--auto]"
}

main() {
  clear
  echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${PURPLE}║                    Ubuntu Server Admin                          ║${NC}"
  echo -e "${PURPLE}║                     Script de Instalação                        ║${NC}"
  echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
  ...
  if [[ "$SKIP_SSL" != true ]]; then
    ...
    log_warning "Configuração SSL pulada"
  fi
...
EOF
  ...
  log_info "Compilando Angular (produção)..."
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then#!/usr/bin/env bash
set -Eeuo pipefail
